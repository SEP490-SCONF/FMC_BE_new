...
namespace ConferenceFWebAPI.Controllers
{
    ...
    public class UserConferenceRolesController : ControllerBase
    {
        ...
        [HttpGet("conference/{conferenceId}/roles-organizer")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<UserDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [EnableQuery]
        public async Task<IActionResult> GetConferenceMembersOrganizer(int conferenceId)
        {
            try
            {
                // Kiểm tra sự tồn tại của Conference
                ...
                var desiredRoleIds = new List<int> { 4 };
